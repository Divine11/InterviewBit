...
def seats(A):
    ...
    for i in range(n):
        if A[i] =='x':
            index.append(i)
    # print(index)
    no_of_people = len(index)
    if no_of_people==0:
        return 0
    median = index[no_of_people//2]
    res = 0
    # print(median)
    median_index = index.index(median)
    #print(median_index)
    i = median_index-1
    current_median_index = median
    while i>=0:
        res += current_median_index-index[i]-1
        current_median_index-=1
        i-=1
    #print(res)
    i = median_index+1
    current_median_index = median
    while i<no_of_people:
        res+= index[i]-current_median_index-1
        current_median_index+=1
        i+=1
    return res%100003
