...
def anagrams(self, A):
    ...
    for i in range(len(A)):
        temp = sorted(list(A[i]))
        temp = ''.join(temp)
        if temp in dic:
            dic[temp].append(counter)
        else:
            dic[temp] = [counter]
        counter+=1
