...
def good_words_trie(good_words):
    lis = good_words.split("_")
    if len(lis)==0:
        return None
    root = TrieNode()
    for i in lis:
        ...
        for j in i:
            indx = char_to_int(j)
            if not temp.children[indx]:
                temp.children[indx] = TrieNode()
...
def print_trie(root,A):
    ...
    for i in range(26):
        if root.children[i]:
            ...
            A.pop()
def get_score_of_string(A,root):
    lis = A.split("_")
    res = 0
    for i in lis:
        temp = root
